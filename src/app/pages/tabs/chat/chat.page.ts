...
import { ChatApiService, MessageModel, RoomListModel } from 'src/app/services/api-wishalink.service';
...
export class ChatPage implements OnInit {
    ...
    ionViewWillEnter() {
        ...
        this.chatService.onNewMessage.subscribe(() => {
            this.currentRoomMessages = this.chatService.roomMessages[this.currentRoom.id];
            setTimeout(() => {
                this.zone.run(() => {
    ...
    loadRoom(roomId: number) {
        ...
        this.zone.run(() => {
            ...
            if (this.currentRoom != null) {
                ...
                if (this.currentRoomMessages == null || this.currentRoomMessages.length == 0) {
                    this.loadMessages();
                }
            }
    ...
    leaveRoom() {
        ...
        this.chatApiService.leaveroom(roomToLeaveId, this.chatService.connectionId)
            .subscribe(
                v => {
                    ...
                    this.zone.run(() => {
                        this.chatService.rooms = this.chatService.rooms.filter(x => x.id != roomToLeaveId);
                    });
    ...
    focusInput() {
        const input = document.querySelector('#newMessageInput input') as HTMLElement;
        if(input) {
            this.zone.run(() => {
                input.focus();
            })
    ...
    sendMessage() {
        ...
        this.chatApiService.createmessage(messageModel)
            .subscribe(
                v => {
                    this.zone.run(() => {
                        this.newMessage = '';
    ...
    loadMessages() {
        ...
        if (this.currentRoomMessages != undefined && this.currentRoomMessages.length > 0) {
            minMessageId = this.currentRoomMessages.map(x => x.id).sort((a, b) => a - b)[0];
        }

        this.isLoading = true;
        this.chatApiService.getmessages(this.currentRoom.id, minMessageId)
            .subscribe(
                v => {
                    this.zone.run(() => {
                        this.isLoading = false;

                        if (v != undefined && v.messages != undefined && v.messages.length > 0) {
